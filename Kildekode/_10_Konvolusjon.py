...
class convolutionDemo:
    def __init__(self, xn, hn, fig_num = 1, figsize=(8, 6)):
        ...
        self.hn_samples = interactiveStem(ax1, np.arange(len(self.hn)), self.hn, color='C3', label=r'$h[n-k]$')
        ...
        self.xn_hn.ax.set_ylim([-max(abs(xn))*max(abs(hn)), max(abs(xn))*max(abs(hn))])
        ...
        ax3.set_ylim(np.array(ax1.get_ylim())*max(abs(np.fft.fft(hn, 100))))
    ...
    def update(self, n):
        ...
        self.xn_hn.update(n=n_array, xn=self.xn[n_1:n_2]*self.hn[-1:-1-noverlap:-1])
